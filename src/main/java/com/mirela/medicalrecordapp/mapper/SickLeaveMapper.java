...
public interface SickLeaveMapper {
    ...
    @Mapping(target = "doctorId", source = "appointment.doctor.id")
    @Mapping(target = "doctorName", expression = "java(s.getAppointment().getDoctor().getUser().getFirstName() + \" \" + s.getAppointment().getDoctor().getUser().getLastName())")
    @Mapping(target = "specialization", source = "appointment.doctor.specialization")
    MySickLeaveDto toDTo(SickLeave s);
}
